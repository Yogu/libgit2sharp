...
namespace LibGit2Sharp.Tests
{
    public class NoteFixture : BaseFixture
    {
        ...
        private void AssertCommitSignaturesAre(Repository repo, string commitish, Signature signature)
        {
            ...
            Assert.Equal(Constants.Signature.Name, commit.Author.Name);
            Assert.Equal(Constants.Signature.Email, commit.Author.Email);
            Assert.Equal(Constants.Signature.Name, commit.Committer.Name);
            Assert.Equal(Constants.Signature.Email, commit.Committer.Email);
        }
